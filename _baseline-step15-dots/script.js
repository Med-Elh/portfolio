...
(function () {
  ...
  /* Someone who has never touched the button is being shown their system
     setting, so the page follows it if it changes mid-visit. Once they
     have chosen, their choice wins and this stops applying. */
  ...
  /* The same path again, inside the mask. It is what actually gets drawn
     now: the visible line is dotted and its dasharray is spoken for. */
  var jrMask = document.getElementById('jr-mask');
  ...
  if (jr && jrLine && jrTail) {
    ...
    function repath() {
      anchors = readAnchors();
      var jrD = ribbon(anchors);

      jrLine.setAttribute('d', jrD);
      /* Byte for byte the same path, or the mask would reveal a shape the
         line does not follow */
      if (jrMask) { jrMask.setAttribute('d', jrD); }
    ...
    function drawJr() {
      ...
      /* The MASK is what draws. Writing these on the visible line would
         overwrite the dot pattern with one dash the length of the whole
         path, and the dots would vanish the moment the first frame ran. */
      if (jrMask) {
        jrMask.style.strokeDasharray = mainLen + ' ' + mainLen;
        jrMask.style.strokeDashoffset = (mainLen * (1 - p)).toFixed(1);
      }
