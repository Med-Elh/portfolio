...
(function () {
  ...
  if (heroEl && !reducedMotion.matches) {
    ...
    /* ---- Rack focus ---------------------------------------------------
       Two blur tracks rather than one, which is the whole point: the
       portrait starts going soft immediately and reaches 20px, while the
       copy stays sharp for the first 45% of the scroll and only ever
       reaches 8px. The gap between the two curves is what reads as a
       lens pulling focus off the background, rather than the whole hero
       dissolving together.

       Both the old opacity curves were 1 -> 0 across the same range,
       which would have hidden the blur behind a fade — you cannot see a
       defocus happen on something that is already transparent. They are
       re-timed below: the background holds at 0.15 until very late, the
       foreground does not begin fading until it starts to blur.

       filter is expensive on a 928x1152 image, so it is desktop only and
       will-change goes on only while the hero is actually mid-scroll. */
    var blurWide = window.matchMedia('(min-width: 900px)');
    var blurBgMax = 20;
    var blurFgMax = 8;
    var blurFgStart = 0.45;
    var navLinks = document.querySelector('.nav__links');
    /* The stat row is left out of the split on purpose (see the note on
       leftSel above) — it neither moves nor fades. It still has to go
       soft, though, or it sits perfectly sharp in the middle of a hero
       that is defocusing around it. Blur only, nothing else. */
    var splitStats = heroEl.querySelector('.stats');
    var blurArmed = false;

    function writeSplit(el, percent, fade, blur) {
      if (!el) {
        return;
      }
    ...
    function applySplit() {
      ...
      /* Above 900px the migration module below owns the hero outright —
         same elements, same properties. Two modules writing one
         element's transform is how you get a fight neither wins. */
      if (document.body.classList.contains('is-migrating')) {
        return;
      }
      ...
      /* Background: soft early, but still faintly there almost all the
         way down, so there is something left to watch go out of focus */
      var bgFade = progress < 0.8
        ? 1 - (progress / 0.8) * 0.85
        : 0.15 * (1 - (progress - 0.8) / 0.2);

      /* Foreground: holds, then goes — the legible half of the effect */
      var fgFade = progress < 0.5 ? 1 : 1 - (progress - 0.5) / 0.5;

      var blurOn = blurWide.matches;
      var bgBlur = blurOn ? blurBgMax * Math.min(1, progress / 0.7) : 0;
      var fgBlur = blurOn
        ? blurFgMax * Math.max(0, (progress - blurFgStart) / (1 - blurFgStart))
        : 0;
      ...
      for (var i = 0; i < splitLeft.length; i++) {
        writeSplit(splitLeft[i], -shift, fgFade, fgBlur);
      }

      writeSplit(splitRight, shift, bgFade, bgBlur);

      if (splitStats) {
        splitStats.style.filter = fgBlur > 0.05
          ? 'blur(' + fgBlur.toFixed(2) + 'px)'
          : '';
      }

      /* The nav rides the foreground track, so its links stay readable
         while the portrait behind them softens. Dropped the moment the
         rail latches — the sidebar must never be blurred. */
      if (navLinks) {
        navLinks.style.filter =
          (fgBlur > 0.05 && !document.body.classList.contains('is-rail'))
            ? 'blur(' + fgBlur.toFixed(2) + 'px)'
            : '';
      }

      /* Promote only while something is actually moving. Leaving
         will-change: filter on permanently keeps the portrait in its own
         composited layer for the whole session for no reason. */
      var midScroll = progress > 0 && progress < 1;

      if (midScroll !== blurArmed) {
        blurArmed = midScroll;

        if (splitRight) {
          splitRight.style.willChange = midScroll ? 'transform, filter, opacity' : '';
        }
      }
    ...
    window.addEventListener('resize', function () {
      ...
      /* Same for the blur: applySplit writes 0 below 900px, but "0" is
         written as an empty string only on the next tick, and a resize
         that lands mid-hero would otherwise leave a frozen blur behind */
      if (!blurWide.matches) {
        for (var b = 0; b < splitLeft.length; b++) {
          splitLeft[b].style.filter = '';
        }

        if (splitRight) {
          splitRight.style.filter = '';
          splitRight.style.willChange = '';
        }

        if (splitStats) {
          splitStats.style.filter = '';
        }
  ...
  /* ------------------------------------------------------------------
     Hero to sidebar — content migration

     The name, the four stat numbers and the six nav links are not two
     sets of elements crossfading. Each is ONE element that begins the
     page sitting exactly where the hero puts it and is carried into the
     sidebar by transform alone, tracked to scroll position the whole
     way — so scrolling back up runs it backwards exactly, with no state
     to get out of step.

     Three things make that work:

     a. The hero is pinned for the length of the run (the stylesheet
        gives .hero-zone 220vh and sticks the hero to the top of it). If
        the hero scrolled normally, the positions the content is leaving
        would slide out from under it mid-flight.

     b. Anything that changes size is rendered at its LARGE size and
        scaled DOWN. Scaling small text up would render it blurred.

     c. Every start and end position is measured from the real layout,
        not written down as numbers here. The sidebar is ordinary CSS
        flow; the transforms simply hold its contents out at the hero
        until the scroll releases them.

     Above 900px with motion allowed. Below that, or under reduced
     motion, none of it runs and the two fallbacks are handled in
     migSetMode below.
     ------------------------------------------------------------------ */

  var migNav = document.getElementById('nav');
  var migHero = document.getElementById('hero');

  if (migNav && migHero) {
    /* The same 900px the rest of the page already treats as "desktop":
       it is where .nav__links come back inline, where the burger goes
       away, and where the hero becomes two columns. Below it the nav is
       the burger and the full-screen overlay menu, the hero is stacked,
       and none of the migration runs — a fixed sidebar on a phone is not
       a smaller version of this idea, it is the wrong idea. */
    var migWide = window.matchMedia('(min-width: 900px)');

    /* Fallback only. The run's real length is measured — see migRunPx
       below — because the distance that matters is not a round number of
       screens, it is "how far until Profile's heading starts revealing".

       The stylesheet pins the hero for 0.7 of a screen (170vh zone). The
       run is usually a little longer than that, so the last of it happens
       while the hero is sliding away and the marquee is coming up. That
       overlap is deliberate: it is what removes the dead stretch. */
    var MIG_ZONE = 0.9;
    var migRunPx = 0;

    var MIG_BLUR = 4;    /* peak blur on the travelling content, px */
    var HERO_BLUR = 12;  /* peak blur on the hero behind it, px */

    var migBrand = migNav.querySelector('.nav__brand');
    var migLinks = [].slice.call(migNav.querySelectorAll('.nav__link'));
    var migSlots = [].slice.call(migNav.querySelectorAll('.rail__stat-value'));
    var migInner = migNav.querySelector('.nav__inner');
    var heroName = migHero.querySelector('.hero__name');
    var heroValues = [].slice.call(migHero.querySelectorAll('.stat__value'));

    var migClones = [];
    var migTracks = [];
    var migMode = '';
    var migMeasured = false;
    var migTicking = false;
    var migSpent = false;
    var migLatched = false;

    /* Close to the stylesheet's own cubic-bezier(0.16, 1, 0.3, 1): most
       of the distance covered early, then a long settle. */
    function migEase(t) {
      return t >= 1 ? 1 : 1 - Math.pow(2, -10 * t);
    }

    /* Each piece travels across its own slice of the zone rather than
       all of them across the whole of it. That stagger is what lets the
       eye follow one link at a time dropping into the column. */
    function migWindow(p, from, to) {
      var t = (p - from) / (to - from);
      return t < 0 ? 0 : (t > 1 ? 1 : t);
    }

    /* Barely there for the first 30%, up through the middle, and fully
       sharp again well before the end. A rack focus is a moment that
       passes, not a state the page sits in. */
    function migBlurAt(p, peak) {
      if (p <= 0) { return 0; }
      if (p < 0.30) { return 0.35; }
      if (p < 0.52) { return 0.35 + (peak - 0.35) * ((p - 0.30) / 0.22); }
      if (p < 0.88) { return peak * (1 - (p - 0.52) / 0.36); }
      return 0;
    }

    /* The copies of the hero's numbers that actually travel. Built here
       rather than written into the HTML: without script they would be
       four stray numbers with nothing to do. */
    function migBuildClones() {
      if (migClones.length || !heroValues.length) {
        return;
      }

      for (var i = 0; i < heroValues.length && i < migSlots.length; i++) {
        var copy = document.createElement('span');
        /* Same class as the hero's own, so it renders identically */
        copy.className = 'stat__value mig-stat';
        copy.setAttribute('aria-hidden', 'true');
        copy.textContent = heroValues[i].textContent;
        migNav.appendChild(copy);
        migClones.push(copy);
      }
    }

    var migSettled = false;

    /* Gates overflow-y on the card. Kept off for the whole run, because
       a scroll container would clip the links and the brand while they
       are still outside the card on their way in. */
    function migSetSettled(on) {
      if (on === migSettled) {
        return;
      }

      migSettled = on;
      migNav.classList.toggle('is-settled', on);
    }

    function migShowClones(on) {
      for (var i = 0; i < migClones.length; i++) {
        migClones[i].style.display = on ? '' : 'none';
      }
    }

    function migClearStyles() {
      var i;

      if (migBrand) {
        migBrand.style.transform = '';
        migBrand.style.width = '';
      }

      /* Written by migMeasure from the brand's finished height — has to
         go back before the next measurement, or each one reserves room
         on top of the last one's reservation */
      if (migInner) {
        migInner.style.paddingTop = '';
      }

      for (i = 0; i < migLinks.length; i++) {
        migLinks[i].style.transform = '';
      }

      for (i = 0; i < migClones.length; i++) {
        migClones[i].style.transform = '';
        migClones[i].style.left = '';
        migClones[i].style.top = '';
      }

      migNav.style.removeProperty('--rail-bg-o');
      migNav.style.removeProperty('--rail-late');
      migNav.style.removeProperty('--mig-blur');
      migHero.style.removeProperty('--hero-fade');
      migHero.style.removeProperty('--hero-blur');
      migHero.classList.remove('is-spent');
      migSpent = false;
    }

    /* The hero-split module above writes transform, opacity and filter
       onto the same hero children. It stands down while migrating, but
       whatever it wrote last has to be cleared or it stays frozen there. */
    function migClearSplit() {
      var sel = ['.hero__intro', '.hero__actions', '.hero__availability',
                 '.hero__media', '.stats'];

      for (var i = 0; i < sel.length; i++) {
        var el = migHero.querySelector(sel[i]);
      ...
      var list = migNav.querySelector('.nav__links');

      if (list) {
        list.style.filter = '';
      }

      migHero.style.pointerEvents = '';
    }

    /* ---- Measurement ------------------------------------------------
       Everything is read off the real layout, twice: once with the nav
       in bar form (where the links start) and once in card form (where
       they end). Only the .is-rail class is toggled for that — never
       .is-migrating, because that one controls the zone's height, and
       collapsing a 220vh element for a moment while the page is scrolled
       would move the scroll position out from under the measurement. */
    function migMeasure() {
      if (!migBrand || !heroName || !migInner) {
        return;
      }
      ...
      /* Bar form: where the links are coming from */
      migNav.classList.remove('is-rail');

      var barLinks = [];

      for (i = 0; i < migLinks.length; i++) {
        barLinks.push(migLinks[i].getBoundingClientRect());
      }

      /* Card form: where everything is going. .is-settled goes on for
         the measurement too — on a short screen it puts a scrollbar in
         the column, and the links have to be measured in the layout they
         will actually come to rest in, not a slightly wider one. */
      migNav.classList.add('is-rail');
      migNav.classList.add('is-settled');

      var navBox = migNav.getBoundingClientRect();

      /* The hero is pinned to the top-left of the viewport for the whole
         run, so an element's start position is just its offset inside
         the hero.

         Position comes from the offsetTop/offsetLeft chain rather than a
         bounding rect, because a rect includes transforms and these
         elements are carrying one: the hero's load-in rise holds every
         [data-rise] child 24px low for the first 800ms. A measurement
         taken during that window — which is exactly when this first runs
         — put every start position 24px below where the content actually
         sits, so the travelling stat numbers came to rest on top of their
         own labels. Offsets are layout positions and ignore transforms,
         so this is right whenever it runs.

         Width and height still come from the rect: the rise is a
         translate, which does not change either. */
      function pinned(el) {
        var x = 0;
        var y = 0;
        var node = el;

        while (node && node !== migHero) {
          x += node.offsetLeft;
          y += node.offsetTop;
        ...
        return { left: x, top: y, width: b.width, height: b.height };
      }

      /* Brand. Rendered at the hero's own display size and given the
         h1's measured width, so the two break their lines in the same
         places, then scaled down to the width of the card's column. */
      var nameBox = pinned(heroName);
      migBrand.style.width = nameBox.width + 'px';

      var brandBox = migBrand.getBoundingClientRect();
      var pad = brandBox.left - navBox.left;
      var column = navBox.width - pad * 2;
      var brandScale = nameBox.width > 0 ? column / nameBox.width : 1;

      /* The name breaks into two lines at most desktop widths, and two
         lines scaled to the full column width is a large share of a short
         screen's card. --rail-brand-max is the stylesheet's ceiling on
         that, tightened at short viewport heights; whichever of the two
         limits bites first wins. */
      var brandMaxRaw = window.getComputedStyle(migNav)
        .getPropertyValue('--rail-brand-max').trim();
      var brandMax = parseFloat(brandMaxRaw);

      /* A custom property's computed value is just the tokens it was
         written with — no unit resolution happens, so "7rem" parses as
         the number 7. Converting has to be done here. */
      if (/rem$/.test(brandMaxRaw)) {
        brandMax *= parseFloat(
          window.getComputedStyle(document.documentElement).fontSize
        ) || 16;
      }

      if (brandMax > 0 && nameBox.height > 0) {
        brandScale = Math.min(brandScale, brandMax / nameBox.height);
      }

      migTracks.push({
        el: migBrand,
        sx: nameBox.left - brandBox.left,
        sy: nameBox.top - brandBox.top,
        es: brandScale,
        from: 0.05,
        to: 0.62
      });

      /* The brand is out of flow, so the column has to be told how much
         room to leave above it. Its finished height depends on how many
         lines the h1 broke into and how far it was scaled down — both
         only known here — so this is measured rather than guessed at in
         the stylesheet. The gap below it is read off the column's own
         row-gap so the short-screen tiers tighten it too, rather than
         this quietly pinning it to the desktop value. Set before
         anything below is read: it moves every row in the card. */
      var brandGap = parseFloat(window.getComputedStyle(migInner).rowGap);
      ...
      migInner.style.paddingTop =
        ((brandBox.top - navBox.top) + nameBox.height * brandScale + brandGap) + 'px';

      /* Numbers. Scale comes from the ratio of the two font sizes rather
         than of the two widths — the text changes while the counters run
         and a width-derived scale would change with it. */
      if (heroValues.length && migSlots.length) {
        var heroFont = parseFloat(window.getComputedStyle(heroValues[0]).fontSize);
        var slotFont = parseFloat(window.getComputedStyle(migSlots[0]).fontSize);
        var statScale = heroFont > 0 ? slotFont / heroFont : 1;

        for (i = 0; i < migClones.length; i++) {
          var slotBox = migSlots[i].getBoundingClientRect();

          migClones[i].style.left = (slotBox.left - navBox.left) + 'px';
          migClones[i].style.top = (slotBox.top - navBox.top) + 'px';

          var cloneBox = migClones[i].getBoundingClientRect();
          var valueBox = pinned(heroValues[i]);

          migTracks.push({
            el: migClones[i],
            sx: valueBox.left - cloneBox.left,
            sy: valueBox.top - cloneBox.top,
            es: statScale,
            lead: true,
            from: 0.12 + i * 0.05,
            to: 0.56 + i * 0.05
      ...
      /* Links. Same type size at both ends, so they only travel. */
      for (i = 0; i < migLinks.length; i++) {
        var railBox = migLinks[i].getBoundingClientRect();

        migTracks.push({
          el: migLinks[i],
          sx: barLinks[i].left - railBox.left,
          sy: barLinks[i].top - railBox.top,
          es: 1,
          /* L-shaped: down to its own row first, then left into the
             column. See migPlace. */
          lead: true,
          from: 0.18 + i * 0.055,
          to: 0.58 + i * 0.055
      ...
      /* ---- How long the run actually is -------------------------------
         Not a fixed number of screens. The run has to finish at the exact
         scroll position where the next section's heading starts revealing,
         or there is a stretch with the sidebar already settled and nothing
         arrived yet — which is the whole complaint this is answering.

         The reveal observer uses rootMargin '0px 0px -10% 0px', so a
         target counts as arrived once its top is above 90% of the viewport
         height. Solving that for scrollY gives the moment to aim at.
         Measured rather than written down, so it stays right at any
         viewport size and survives the content above it changing. */
      var viewH = window.innerHeight || 1;

      /* Whatever is the first thing below the hero that animates itself
         in — not a named section. Written this way so that moving a
         section in above Profile (the pitch band is exactly that) moves
         the finish line with it, instead of stretching the run across
         the new section and reopening the gap it was tuned to close.
         Falls back to Profile's marker if nothing above it reveals. */
      var marker = document.querySelector('.hero-zone ~ * [data-reveal]') ||
                   document.querySelector('#profile .section__marker');

      if (marker) {
        var markerDocY = marker.getBoundingClientRect().top + window.scrollY;
        migRunPx = markerDocY - viewH * 0.9;
      } else {
        migRunPx = 0;
      }

      /* Floors it against a short viewport or a missing marker — a run
         that collapses towards zero would snap rather than travel */
      if (!(migRunPx > viewH * 0.5)) {
        migRunPx = MIG_ZONE * viewH;
      }

      /* Back to whatever the current scroll position actually calls for;
         migApply sets it properly on the same tick */
      migNav.classList.toggle('is-settled', migSettled);

      migMeasured = true;
    }

    /* t = 0 is the hero position at full size, t = 1 is the sidebar.
       Nothing else is stored, which is why the reverse needs no code of
       its own: scroll up and p simply comes back down.

       Tracks marked `lead` move on an L-shaped path rather than a
       straight diagonal: vertical first, horizontal second. Six links
       leaving a horizontal bar for a vertical column all converge on the
       same narrow strip, and on a straight line they cross each other on
       the way — which is what produced the pile-up of overlapping link
       text mid-scroll. Dropping each one onto its own row before any of
       them slides left means no two are ever at the same height while
       they are moving, so they cannot collide however cramped the bar
       they started from was. */
    function migPlace(track, p) {
      var t = migWindow(p, track.from, track.to);
      var ease = migEase(t);
      var tx = ease;
      var ty = ease;

      if (track.lead) {
        ty = migEase(migWindow(t, 0, 0.62));
        tx = migEase(migWindow(t, 0.34, 1));
      }

      var x = track.sx * (1 - tx);
      var y = track.sy * (1 - ty);
      var s = 1 + (track.es - 1) * ease;
    ...
    function migSetMode(mode) {
      ...
      if (mode === 'migrate') {
        document.body.classList.add('is-migrating');
        migNav.classList.add('is-rail');
        migShowClones(true);
        migClearSplit();
        migMeasured = false;
      ...
      document.body.classList.remove('is-migrating');
      migClearStyles();
      migShowClones(false);
      migSetSettled(false);
      migMeasured = false;
      ...
      if (mode === 'static') {
        /* Reduced motion. The sidebar is a layout, not an effect, so it
           still forms — it simply arrives, with nothing travelling and
           no pinned zone for it to travel across. */
        migNav.style.setProperty('--rail-bg-o', '1');
        return;
      }

      /* Narrow: the plain top bar, exactly as it has always been */
      migNav.classList.remove('is-rail');
    ...
    function migApply() {
      ...
      if (reducedMotion.matches) {
        ...
        var past = window.scrollY > viewH * 0.5;

        if (past !== migLatched) {
          migLatched = past;
          migNav.classList.toggle('is-rail', past);
          document.body.classList.toggle('is-rail', past);
        }

        /* Nothing ever travels in this mode, so there is never a moment
           where clipping would be wrong */
        migSetSettled(past);
      ...
      /* Two readings of the same number. Everything that travels uses the
         clamped one — the run is over at 1 and the transforms must stop
         there. The hero's own fade uses the raw one, because it keeps
         going for a moment afterwards: see the fade below. */
      var pRaw = window.scrollY / (migRunPx || (MIG_ZONE * viewH));
      var p = pRaw;

      if (p < 0) { p = 0; }
      if (p > 1) { p = 1; }

      /* The counters animate the hero's numbers up from zero, and the
         copies are what is actually on screen — so they have to follow */
      for (i = 0; i < migClones.length; i++) {
        if (migClones[i].textContent !== heroValues[i].textContent) {
          migClones[i].textContent = heroValues[i].textContent;
        }
      }

      for (i = 0; i < migTracks.length; i++) {
        migPlace(migTracks[i], p);
      }

      /* The card's surface is solid at 0.40; the hero is not gone until
         0.78. In that order, deliberately — the travelling content is
         never legible-on-top-of-hero-content, it always has its own
         background underneath it by the time the two overlap.

         The run now ends exactly where Profile's heading starts revealing
         (see migRunPx), so the hero has to still be on screen at 1.0 and
         gone immediately after — it reaches zero at 1.05, a sliver past
         the end, so the two cross rather than leaving a hole between
         them. The hero unpins before that and is physically sliding away
         as it goes, which is what fills the stretch the marquee strip is
         too short to fill on its own.

         Going soft rather than disappearing is also what keeps the
         travelling links readable while they cross it. */
      var fade = 1 - migWindow(pRaw, 0.15, 1.05);

      migNav.style.setProperty('--rail-bg-o', migWindow(p, 0.15, 0.40).toFixed(3));
      migNav.style.setProperty('--rail-late', migWindow(p, 0.62, 0.92).toFixed(3));
      migNav.style.setProperty('--mig-blur', migBlurAt(p, MIG_BLUR).toFixed(2) + 'px');

      migHero.style.setProperty('--hero-fade', fade.toFixed(3));
      migHero.style.setProperty('--hero-blur', migBlurAt(p, HERO_BLUR).toFixed(2) + 'px');

      /* Faded out means not painted at all — a full-viewport blurred
         layer is not worth compositing for the rest of the page */
      var spent = fade <= 0.002;

      if (spent !== migSpent) {
        migSpent = spent;
        migHero.classList.toggle('is-spent', spent);
      }
      ...
      if (latch !== migLatched) {
        migLatched = latch;
        document.body.classList.toggle('is-rail', latch);
      }

      /* Everything has landed: the card can become a scroll container
         now without clipping anything still in flight */
      migSetSettled(p >= 0.999);
    }

    function migQueue() {
      if (!migTicking) {
        migTicking = true;
        requestAnimationFrame(migApply);
      }
    }

    function migRemeasure() {
      migMeasured = false;
      migQueue();
    }

    /* The stat counters animate the hero's numbers up from zero, and the
       copies are what is actually on screen. Most of that count happens
       at the top of the page with nobody scrolling, so a scroll-driven
       sync alone would leave the copies frozen at whatever number the
       last scroll event caught. Watching the originals instead catches
       every step of it. */
    if (window.MutationObserver && heroValues.length) {
      var migWatcher = new MutationObserver(migQueue);

      for (var hv = 0; hv < heroValues.length; hv++) {
        migWatcher.observe(heroValues[hv], {
          childList: true,
          characterData: true,
          subtree: true
        });
    ...
    migApply();
    window.addEventListener('scroll', migQueue, { passive: true });
    window.addEventListener('resize', migRemeasure);
    window.addEventListener('load', migRemeasure);

    /* Text widths decide where everything lands, and Inter arrives after
       the first paint. Measuring before it does would aim all of this at
       the fallback font's metrics. */
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(migRemeasure);
    }
  }
  ...
  /* ------------------------------------------------------------------
     Selected Work — the connected timeline

     Replaces the horizontal card row and everything that drove it: the
     drag, the wheel handler, the arrow buttons, the progress bar and the
     snap suspension are all gone. This section is now ordinary vertical
     page scrolling, so there is nothing left to hijack.

     What remains is a thread. Its shape is measured rather than written
     down: script reads where each marker actually lands and builds one
     cubic segment per gap, so the curve is right whatever height the
     cards turn out to be — including after one of them is opened.

     Drawing it is stroke-dasharray set to the path's own length with
     stroke-dashoffset moved by scroll position. Because the offset is a
     pure function of where the page is, scrolling back up un-draws it
     exactly.
     ------------------------------------------------------------------ */

  var tl = document.getElementById('tl');
  var tlThread = document.getElementById('tl-thread');
  var tlTrack = document.getElementById('tl-track');
  var tlDrawn = document.getElementById('tl-drawn');

  if (tl && tlThread && tlTrack && tlDrawn) {
    var tlCards = [].slice.call(tl.querySelectorAll('.tl-card'));
    var tlDots = [].slice.call(tl.querySelectorAll('.tl-card__dot'));

    /* How far down the viewport a card counts as the one being read.
       Slightly above the middle, which is where the eye actually sits. */
    var TL_FOCUS = 0.55;
    ...
    /* ---- Build --------------------------------------------------------
       One cubic per gap, with both tangents vertical: the thread leaves a
       marker heading straight down and arrives at the next one heading
       straight down, so all the sideways travel happens in the middle of
       the segment and the joins never kink. When the markers share an x
       (below 900px, where the cards do not alternate) the same curve
       flattens into a straight line on its own. */
    function tlBuild() {
      tlReady = false;
      ...
      tlThread.setAttribute('viewBox', '0 0 ' + w + ' ' + h);
      tlThread.setAttribute('width', w);
      tlThread.setAttribute('height', h);
      ...
      for (i = 0; i < tlDots.length; i++) {
        var dot = tlDots[i].getBoundingClientRect();
        var x = dot.left - box.left + dot.width / 2;
        var y = dot.top - box.top + dot.height / 2;
      ...
      for (i = 1; i < points.length; i++) {
        var from = points[i - 1];
        var to = points[i];
        var bend = (to[1] - from[1]) * 0.45;

        d += ' C ' + from[0].toFixed(1) + ' ' + (from[1] + bend).toFixed(1) +
             ', ' + to[0].toFixed(1) + ' ' + (to[1] - bend).toFixed(1) +
             ', ' + to[0].toFixed(1) + ' ' + to[1].toFixed(1);
      }

      tlTrack.setAttribute('d', d);
      tlDrawn.setAttribute('d', d);

      tlLen = tlDrawn.getTotalLength();
      tlDrawn.style.strokeDasharray = tlLen.toFixed(1);
      tlDrawn.style.strokeDashoffset = tlLen.toFixed(1);

      tlReady = true;
    }

    /* The path snakes, so its length is not proportional to its height and
       a percentage of one is not a percentage of the other. Rather than
       approximate, ask the path itself: fourteen halvings find the length
       whose point sits on the focus line, well inside a pixel. */
    function tlLengthAtY(targetY) {
      var lo = 0;
      var hi = tlLen;
      ...
      for (var i = 0; i < 14; i++) {
        ...
        if (tlDrawn.getPointAtLength(mid).y < targetY) {
          lo = mid;
        } else {
          hi = mid;
        }
    ...
    function tlApply() {
      ...
      var box = tl.getBoundingClientRect();
      /* The focus line, in the timeline's own coordinates */
      var focusY = (window.innerHeight || 1) * TL_FOCUS - box.top;
      var last = tlDotY.length - 1;
      var drawn;

      if (reducedMotion.matches) {
        /* The thread is the section's structure, not decoration — it is
           still drawn, it just is not tied to the scrollbar */
        drawn = tlLen;
      } else if (focusY <= tlDotY[0]) {
        drawn = 0;
      } else if (focusY >= tlDotY[last]) {
        drawn = tlLen;
      } else {
        drawn = tlLengthAtY(focusY);
      }

      tlDrawn.style.strokeDashoffset = (tlLen - drawn).toFixed(1);

      /* Whichever marker the thread has most recently reached is the card
         being read. Before the first one, the first card is next up
         rather than nothing being current — a column where every card is
         stepped back reads as broken rather than as focused. */
      var index = 0;

      for (var i = 0; i < tlDotY.length; i++) {
        if (focusY >= tlDotY[i] - 60) {
          index = i;
        }
      }

      if (index !== tlCurrent) {
        if (tlCards[tlCurrent]) {
          tlCards[tlCurrent].classList.remove('is-current');
        }

        if (tlCards[index]) {
          tlCards[index].classList.add('is-current');
        }
    ...
    /* ---- Opening a case study ----------------------------------------
       The first two points of each study are always visible; this opens
       the rest of them, which were the remaining bullets on the old card.
       Nothing is hidden from the page that was not there before. */
    for (var c = 0; c < tlCards.length; c++) {
      (function (card) {
        var button = card.querySelector('.tl-card__toggle');
        ...
        button.addEventListener('click', function () {
          ...
          var text = button.querySelector('.tl-card__toggle-text');

          card.classList.toggle('is-open', open);
          button.setAttribute('aria-expanded', open ? 'true' : 'false');

          if (text) {
            text.textContent = open ? 'Less' : 'More';
          }

          /* The card has just changed height, so every marker below it
             has moved and the thread no longer joins them. Rebuilt once
             now and once when the opening has finished, since it is a
             transition rather than an instant change. */
          tlRebuild();
          window.setTimeout(tlRebuild, 450);
        });
      })(tlCards[c]);
    }

    tlApply();
    window.addEventListener('scroll', tlQueue, { passive: true });
    window.addEventListener('resize', tlRebuild);
    window.addEventListener('load', tlRebuild);

    /* Card heights depend on how the text wraps, and Inter lands after
       the first paint — measuring before it does aims the thread at the
       fallback font's line breaks. */
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(tlRebuild);
    }
  }
  /* ------------------------------------------------------------------
  ...
  if (bgCanvas && bgCanvas.getContext && !reducedMotion.matches) {
    ...
    if (hoverCapable.matches) {
      ...
      if (cursorDot && cursorRing && cursorGlow) {
        ...
        var CURSOR_GROW_SELECTOR = 'a, button, .video__card, .tl-card';
