...
(function () {
  ...
  /* ------------------------------------------------------------------
     Motion is the site's own setting, not the operating system's

     This used to be matchMedia('(prefers-reduced-motion: reduce)') and
     every module below still reads it the same way, through .matches, so
     nothing else in this file changed.

     What changed is who decides. On iOS, Reduce Motion is switched on far
     more often than people expect, frequently for reasons that have
     nothing to do with this page, and it was leaving the site looking
     static and half-built for visitors who had not asked this site for
     anything. So the site now animates by default and the class html.rm
     is what turns motion down, set by the visitor through the control in
     the footer and remembered in localStorage.

     The honest cost, written down so nobody has to rediscover it: a
     visitor with a vestibular disorder who has set Reduce Motion at the
     OS level now gets the full parallax on arrival, and has to find the
     footer control to turn it off. That is why the control exists, why
     it is a real button with a label rather than an icon, and why it is
     in the footer of every page rather than buried in a menu.

     Everything in style.css that used to sit inside a
     prefers-reduced-motion media query is now scoped to html.rm instead.
     Those rules are unchanged; they simply wait for the class. */
  var reducedMotion = {
    matches: document.documentElement.classList.contains('rm')
  };

  function setReducedMotion(on) {
    reducedMotion.matches = !!on;
    document.documentElement.classList.toggle('rm', !!on);

    try {
      localStorage.setItem('motion', on ? 'reduced' : 'full');
    } catch (e) { /* private mode; the choice lasts the visit */ }

    var btns = document.querySelectorAll('[data-motion-toggle]');

    for (var i = 0; i < btns.length; i++) {
      btns[i].setAttribute('aria-pressed', on ? 'true' : 'false');
      var label = btns[i].querySelector('[data-motion-label]');
      if (label) { label.textContent = on ? 'Motion: reduced' : 'Motion: full'; }
    }
  }
  ...
  (function () {
    ...
    /* Sync the label with whatever the inline head script decided */
    setReducedMotion(reducedMotion.matches);
  }());
  ...
  for (var sr = 0; sr < smoothRails.length; sr++) {
    (function (rail) {
      ...
      /* ---- stack mode ----
         Below 900px the row stops being a row. Every card sits in the
         same place and one is shown at a time, so the engine tracks an
         INDEX rather than a pixel offset and each card carries its own
         transform. The pixel machinery above is left alone rather than
         made conditional everywhere: apply() forks once, at the top. */
      var deck = rail.querySelector('[data-vid-deck]');
      var navPrev = rail.querySelector('[data-vid-prev]');
      var navNext = rail.querySelector('[data-vid-next]');
      var count = rail.querySelector('[data-vid-count]');
      var stackMQ = window.matchMedia('(max-width: 899px)');
      var stacked = stackMQ.matches;
      var index = 0;
      var dragDx = 0;

      function cards() { return row.children; }

      /* Where each card sits, given the active index and how far the
         finger has dragged. Percentages of the card's own width, so the
         same numbers hold at every phone size. */
      function layout() {
        var items = cards();
        var w = view.clientWidth || 1;
        var slide = (dragDx / w) * 100;
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (d < 0) {
            /* Gone: off to the left, out of the way and untouchable. */
            tx = -118; sc = 0.94; op = 0; z = 0;
          } else if (d === 0) {
            tx = slide; sc = 1; op = 1; z = 30;
          } else if (d <= 2) {
            /* The two behind. As the front card is dragged away they
               come forward in proportion, so the stack feels connected
               to the finger rather than waiting for it to finish. */
            var lift = Math.min(1, Math.abs(slide) / 55);

            tx = 5 * d - (d === 1 ? 5 * lift : 0);
            sc = (1 - 0.06 * d) + (d === 1 ? 0.06 * lift : 0);
            op = (d === 1 ? 0.55 : 0.28) + (d === 1 ? 0.45 * lift : 0);
            z = 30 - d;
          } else {
            tx = 10; sc = 0.86; op = 0; z = 0;
          }

          el.style.transform = 'translate3d(' + tx.toFixed(2) + '%, 0, 0) scale(' + sc.toFixed(3) + ')';
          el.style.opacity = op.toFixed(3);
          el.style.zIndex = z;
          /* Only the front card is clickable, or a tap could open a
             video the visitor cannot even see. */
        ...
        if (navNext) { navNext.disabled = index >= items.length - 1; }

        if (fill) {
          var last = Math.max(1, items.length - 1);
          fill.style.transform = 'scaleX(' + (index / last).toFixed(4) + ')';
        }
      }

      function goTo(n) {
        var itemsLen = cards().length;

        index = Math.max(0, Math.min(itemsLen - 1, n));
        dragDx = 0;
        layout();
      }

      /* The cards keep the inline styles layout() wrote when the window
         crosses back over 900px, and those would sit on top of the
         side-by-side rules. Clear them. */
      function clearStack() {
        var items = cards();
      ...
      if (stackMQ.addEventListener) {
        stackMQ.addEventListener('change', function (e) { setMode(e.matches); });
      } else if (stackMQ.addListener) {
        stackMQ.addListener(function (e) { setMode(e.matches); });
      }

      if (navPrev) { navPrev.addEventListener('click', function () { goTo(index - 1); }); }
      if (navNext) { navNext.addEventListener('click', function () { goTo(index + 1); }); }
      ...
      row.addEventListener('pointerdown', function (event) {
        ...
        /* Takes the transition off the cards so they track the finger
           1:1. Without it every card would lag the touch by its own
           280ms and the drag would feel like treacle. */
        if (stacked) { dragDx = 0; row.classList.add('is-swiping'); }
      ...
      row.addEventListener('pointermove', function (event) {
        ...
        /* Stack mode: the front card follows the finger and the two
           behind it come forward in proportion. No pixel offset, no
           loop; layout() is the whole of it. */
        if (stacked) {
          var items = cards().length;
          var atEnd = (dx < 0 && index >= items - 1) || (dx > 0 && index <= 0);

          /* Rubber band at both ends, the same third-speed as the row. */
          dragDx = atEnd ? dx / RUBBER : dx;

          moved += Math.abs(event.clientX - lastX);

          var dtS = event.timeStamp - lastT;

          if (dtS > 0) {
            var vS = -(event.clientX - lastX) * (16.7 / dtS);
            vel = vel * 0.7 + vS * 0.3;
          }
      ...
      function release(event) {
        ...
        /* ---- stack: one card, decided by direction ----
           A quarter of the view, or a flick. Below both the card springs
           back to where it was, which is what makes a half-hearted drag
           feel like a question rather than a mistake. */
        if (stacked) {
          var swipe = -dragDx;              /* positive = swiped forward */
          var gate = view.clientWidth * 0.25;
          var step = 0;

          if (swipe > gate || vel > PAGE_VEL) { step = 1; }
          else if (swipe < -gate || vel < -PAGE_VEL) { step = -1; }
      ...
      row.addEventListener('wheel', function (event) {
        ...
        /* The stack has no continuous position to nudge, and hijacking a
           vertical wheel over it would trap the page. */
        if (stacked) { return; }
      ...
      row.addEventListener('keydown', function (event) {
        ...
        if (stacked) { goTo(index + (event.key === 'ArrowRight' ? 1 : -1)); return; }
