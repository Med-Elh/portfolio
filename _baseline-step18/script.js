...
(function () {
  ...
  if (hz && hx && sb) {
    ...
    var nameSO = 0;      /* --name-so in em, 0 in dark */
    var nameTheme = null;
    ...
    function readNameRGB() {
      ...
      /* The outline set. Read in em and kept in em: the pill is scaled by
         a CSS transform for the whole flight, so an em-based stroke rides
         that scale for free and stays the right thickness relative to the
         letters at every size on the way. A px value would have to be
         divided by the scale on every frame. */
      nameInk = cs.getPropertyValue('--name-ink').trim() || 'transparent';
      nameSW = parseFloat(cs.getPropertyValue('--name-sw')) || 0;
      nameSO = parseFloat(cs.getPropertyValue('--name-so')) || 0;
    }
    ...
    function apply() {
      ...
      if (sbLogoIn) {
        ...
        /* The light-theme outline leaves on exactly the window the colour
           ...
           mud. In dark nameSW is 0 and both writes are no-ops. */
        var out = 1 - ink;

        sbLogoIn.style.webkitTextStrokeWidth = (nameSW * out).toFixed(4) + 'em';
        sbLogoIn.style.webkitTextStrokeColor = nameInk;
        sbLogoIn.style.textShadow = nameSO
          ? (nameSO * out).toFixed(4) + 'em ' + (nameSO * out).toFixed(4) +
            'em 0 ' + nameInk
          : 'none';
  ...
  if (mb && mbSvg && mbTrail) {
    ...
    /* The light-theme outline copies. They sit under the clipped yellow and
       have to carry the SAME size and the SAME baseline as the two nodes in
       the clipPath, or the rim drifts off the letters. mbFit() writes to
       them alongside the originals rather than them being sized separately:
       two fitters reading each other is how a name walks itself down to
       nothing over a few resizes. */
    var mbInk1 = [].slice.call(mbSvg.querySelectorAll('[data-mb-ink="1"]'));
    var mbInk2 = [].slice.call(mbSvg.querySelectorAll('[data-mb-ink="2"]'));
    ...
    function mbFit() {
      ...
      if (widest > target * 0.5 && widest < target * 2) {
        ...
        mbInkSize(mbInk1, size);
        mbInkSize(mbInk2, size);
      }
      ...
      var y1 = (top + lead * 0.78).toFixed(1);
      var y2 = (top + lead + lead * 0.78).toFixed(1);

      mbT1.setAttribute('y', y1);
      mbT2.setAttribute('y', y2);
      mbInkY(mbInk1, y1);
      mbInkY(mbInk2, y2);
    }

    function mbInkSize(list, size) {
      for (var i = 0; i < list.length; i++) { list[i].style.fontSize = size + 'px'; }
    }

    function mbInkY(list, y) {
      for (var i = 0; i < list.length; i++) { list[i].setAttribute('y', y); }
    }
