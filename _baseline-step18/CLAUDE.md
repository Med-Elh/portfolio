...
#### The name is yellow in both themes, and outlined in light (STEP 18)
...
**12.28:1**, and against the yellow it is **14.47:1**, so every edge of
every glyph is a hard boundary and the name reads as a sticker rather
than as text. On the dark page the yellow is 14.22:1 on its own and the
outline is switched off entirely.

Five tokens, so the whole treatment is tuned from one place and flips on
its own: `--name`, `--name-ink`, `--name-sw`, `--name-so`,
`--name-shadow`, plus `--name-ink-on` for the SVG copies. In dark every
one of them is neutralised rather than the rules being duplicated.

**`--name-sw` is a stroke width, not the visible rim.** Everywhere it is
used the stroke is painted BEHIND the fill, so the inner half is covered
and only half shows. On the HTML wordmark that is `paint-order: stroke
fill`; in the SVG divider it is two extra `<text>` nodes underneath, one
offset for the hard shadow and one in register for the rim. Painting
behind is not a detail: a centred `-webkit-text-stroke` eats into the
glyph, and Inter 800 at -0.02em tracking cannot spare it, which is the
thinning the layered fallback exists to avoid. Behind, the letter keeps
its designed weight and gains a rim for free.

**The cap on `--name-sw` is the tracking, not taste.** The wordmark runs
at -0.02em and the divider at -0.045em, so the letters are already closer
than the face intends and an outline grows both sides of every gap.
Measured, 0.030em is where the tightest pair in ELHAYYANY still clears.
Past that they weld together.

The divider's ink copies are sized and positioned by `mbFit()` in the
same call that sizes the clip text, never separately: two fitters reading
each other is how a name walks itself down to nothing over a few resizes.

**The flying copy winds the outline down as the pill forms.** script.js
writes the stroke and shadow onto `.sb__logoin` scaled by `1 - ink`, on
exactly the window that turns the letters to ink, in `em` so the values
ride the flight's CSS scale for free instead of needing to be divided by
it every frame. What lands in the sidebar is the ordinary pill. **The
sidebar logo pill at rest is not part of this**: it is dark text on a
yellow lozenge and already has all the contrast it needs.

Checked at 1280, 1440, 1920 and 390: stroke and shadow scale
proportionally with the font, zero horizontal overflow, no letters
touching, and the collage trail still reads inside the outlined letters.
...
one spanning 92% of the SVG's 1000-unit viewBox. In light it carries the
near-black outline and hard shadow described under the Hero wordmark
above, drawn as two extra `<text>` copies UNDER the clipped yellow: SVG
has no `-webkit-text-stroke`, and the visible yellow here is a clipped
`<rect>` with no glyph of its own to stroke, so the layered approach is
not a fallback in this one place, it is the only way.
