...
(function () {
  ...
  /* ------------------------------------------------------------------
     ...
     links, buttons, channel cards and work cards. A separate glow div
  ...
  /* The two Channels cards come up one after another when the grid
     ...
     The count-up runs under reduced motion too. It is a number filling
    ...
    function chRun() {
      ...
      for (var i = 0; i < chCounts.length; i++) {
        (function (el) {
          ...
          /* The markup already holds the finished figure, so a browser
             that never reaches this line shows the real number. Winding
             it back is the first thing the run does, and only ever
             immediately before it starts counting. */
          el.textContent = '0';

          /* 260ms in. The card's own entrance is 600ms, and a number
             that starts while its card is still arriving reads as two
