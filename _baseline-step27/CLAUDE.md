...
Safari reads, and without both a drag starting on a video thumbnail drags
the picture out of the page instead of moving the carousel.
...
a yellow chip, and the UGC band's marker inheriting the page's grey
...
The lightbox, its arrows, dots and swipe are unchanged and cover all six
automatically: the module finds its cards by [data-video-id], so there is
no count written down.

**This row has its own engine (STEP 24).** It carries `data-rail-smooth`,
which makes the shared `[data-rail]` module skip it and hands it to the
transform engine below that module in script.js. What People Say keeps
the shared one: its dash indicator reads `scrollLeft` and its drag limits
were measured against that, so rewriting a single engine for both would
have put verified behaviour at risk to fix a different row. Two engines
is the cost of that, and it is deliberate.

The difference is who owns the position. A native scroller is owned by
the browser and lands on the exact pixel the gesture asks for, which is
what made this row feel like it jumped. Here a gesture only ever moves a
TARGET, and one rAF loop walks the real offset 12% of the way toward it
each frame. Drag, wheel, trackpad, touch, keyboard and the arrows all do
the same one thing: move the target. The loop stops itself once it
arrives, so an idle page runs no animation frames.

**Below 900px the row is not a row, it is a STACK (STEP 27).** Side by
side, one card was 332px of a 370px viewport: a whole screen for one
video with a 24px sliver of the next, which read as a layout that had not
quite fitted rather than as a deliberate one. Stacked, the card is the
screen and the two behind it say how many are left.

The engine forks once, at the top of `apply()`: in stack mode it tracks
an INDEX and writes a transform per card, rather than tracking a pixel
offset and writing one transform on the track. Everything else, the
measuring, the lerp loop, the momentum, is left alone and simply unused
there. `.vid__row` gets `transform: none !important` in that mode, or the
track's own offset would move the whole stack as well.

**Horizontal swipe, not vertical.** A vertical swipe is the gesture
people know from Shorts, but it is also the gesture that scrolls the
page, and taking it would trap the one thing a visitor needs to leave the
section. Left and right are free, which is why `touch-action` stays
`pan-y`. Verified: the page still scrolls vertically over the stack.

The settle is a CSS transition on the cards, not a rAF loop: it is one
interruptible move of two properties, and a transition retargets from
wherever the finger left it. `.is-swiping` takes the transition off while
a finger is down so the card tracks it 1:1 rather than lagging 280ms.

The cards behind come forward in proportion to the drag, so the stack
feels connected to the finger rather than waiting for it to finish. Only
the front card has pointer events, or a tap could open a video that is
not visible.

A quarter of the view, or a flick, advances it; below both it springs
back. There are also arrow buttons, which disable at the ends, a counter,
and a swipe hint that nudges three times and then stops asking. The rail
drops its `padding-left` and negative `margin-right` there, because those
exist to start a ROW on the wrap edge and run it off screen, and a stack
is centred: with them the cards sat 61px from the left and 9px from the
right of a 390px screen.

**Touch pages, one card per swipe, whatever the speed (STEP 26).** This
still governs the desktop-width touch case; below 900px the stack above
takes over. Free
...
back. Under reduced motion the lerp is 1, momentum is off and the rubber
band is off: it arrives instantly, in the right place.

**`scrollWidth` leaves a flex container's trailing padding out**, so the
travel is measured from the last card's right edge plus the computed
`padding-right`, never from `scrollWidth`. Trusting it gave 820 against
the 896 the content actually occupies and left the last card flush with
the screen edge. Measured: leading and trailing gutters both 36px at 390.

The row needs a clipping VIEWPORT (`.vid__view`, `data-rail-view`),
because an element cannot both clip its overflow and be the thing that
moves. `overflow-x` on the track itself has to go with it, or a trackpad
swipe moves the row twice: once by transform and once natively
underneath.

Measured at 1440 dragging at about 690 px/s: median frame 16.6ms, p95
18.5ms, 0.5% dropped. The engine's glide on its own has 0% of frames
over 20ms.

The cards were ALREADY a thumbnail plus a play button, with the iframe
built only when the lightbox opens, so nothing changed there. What was
added is a decode before the row is reachable: `loading="lazy"` on a
HORIZONTAL row means the cards off to the right have not loaded when a
drag starts, and they pop in mid-gesture.

The progress bar is violet fill on a #333 track, scaled with `transform:
scaleX()` rather than resized, because width is a layout property and
this runs on every frame of the loop.
